...
module.exports = function (db) {
  ...
  router.get('/', function (req, res) {
    ...
    let count = 1
    let syntax = []
    let sql_count = 'SELECT count(*) AS total FROM varian'
    let sql = 'SELECT * FROM varian'
    if (cari_id) {
      sql += ' WHERE '
      search.push(`%${cari_id}%`)
...
INNER JOIN gudang gud ON gud.id_gudang = var.id_gudang WHERE id_varian = $1;`, [req.params.id], (err, rows) => {
      ...
      db.query('SELECT * FROM satuan', (err, rowsS) => {
        db.query('SELECT * FROM gudang', (err, rowsG) => {
          db.query('SELECT * FROM barcode_varian()', (err, rowsC) => {
            ...
            res.render('varian_add', { currentDir: 'varian', current: '', barang, satuan, gudang, barcode });
          })
        })
      })
    })
  ...
  router.get('/edit/:id', (req, res) => {
    ...
    db.query('SELECT * FROM varian WHERE id_varian = $1', [req.params.id], (err, rows) => {

      if (err) {
        return console.error(err.message);
      }
      res.render('varian_edit', { rows: rows.rows, currentDir: 'varian', current: '' });
    })
  })
